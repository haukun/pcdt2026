...
URL に `?work=<id または 名前>` を付けると、その作品だけを表示し続ける（自動切り替えなし）。

- 検索順（`findWork`、大小無視）: id 完全一致 → title 完全一致 → id/title の部分一致。
  例: `?work=tp5_210101_ShineFlower` / `?work=ShineFlower` / `?work=Butterfly`。
- 対象は **全作品（manifest 全体）** から探す。除外リストの作品でも明示指定なら表示する。
- ただし p5 バージョンが未確定/未用意の作品は表示不可（通知を出す）。
- 自動遷移・進捗バーは無効。グリッド作品（`rerunMs`）はその場での再実行は継続する。
- `typing` / `duration` パラメーターと併用可（duration は固定表示では実質無効）。
...
- **固定表示モード**: `?work=<id or 名前>` で指定作品だけを表示し続ける（自動切替なし）。
