...
- **額縁**: `#frames::before` を作品の外側（`inset: -10px`）に置き、細いダーク基調の枠＋
  落ち影＋内側の微ハイライトで額縁を表現する。作品を主役にするため控えめ。作品サイズに追従する。
...
- **タイトル**（`.plate-board` / `#plate-title`）はプレート幅の**中央**（`margin: 0 auto`）。
- **バッジ群**（`#plate-tags`、イベントタグ + p5バージョン）は**作品枠の右端**に絶対配置で
  縦積み・右揃え。絶対配置のため、バッジの幅・個数はタイトルの中央位置に影響しない。
- **作品に重ねない/縦中央に配置**: 利用可能領域は「上余白 `TOP_MARGIN(24)`（カードと揃える）〜
  下部 `PLATE_RESERVE(84)`」。`fitStage` は額縁厚み `FRAME_BORDER(10px)` を織り込み、
